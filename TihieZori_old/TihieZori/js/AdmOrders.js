...
var changeTitle = function () {
    this.title = function () {
        ...
        document.title = (title == "Заказ!!!" ? "" : "Заказ!!!");
    }
};

changeTitle.prototype.blurred = false;

changeTitle.prototype.start = function ()
{
    var audio = new Audio();
    audio.src = this.blurred ? "img/GudokLong.mp3" : "img/GudokShort.mp3";
    audio.autoplay = true;
    this.timer = this.blurred && setInterval(this.title, 700)
};

changeTitle.prototype.stop = function () { clearInterval(this.timer) };

var timerTitle = new changeTitle();

window.onblur = function () { timerTitle.blurred = true };
window.onfocus = function () { timerTitle.stop() };
