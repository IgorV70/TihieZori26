...
class TextControl {
    ...
    SetValue(val) { if (!val) val = ""; this.el.text(val); }
}
...
class CTable {
    ...
    Init() {
        $.getJSON('head/User.dbo', headline => {
            $.getJSON('data/User.dbo', data => {
                var s1 = $(".s1");
                s1.hide();
                var head = this.head;
                ...
                this.InitForm();
                $("#add").click(event => {
                    $.getJSON('add/User.dbo', { "DatM": new Date(), "Title": "Объявление", "Comment": "Текст объявления" }
        ...
        this.Actions = {
            ...
            del: row => {
                ...
                if (this.BeforeDelete(data)) {
                    $.getJSON('del/User.dbo', { "Id": data[this.head["Id"]] }, () => {
                        row.remove();
                    });
                };
            },
...
var docTable = new CTable({ saveurl: 'edit/User.dbo' });
