...
class CTable {
    ...
    Init() {
        $.getJSON('head/Advert.dbo', headline => {
            $.getJSON('data/Advert.dbo', data => {
                var s1 = $(".s1");
                s1.hide();
                var head = this.head;
                ...
                this.InitForm();
                $("#add").click(event => {
                    $.getJSON('add/Advert.dbo', { "DatM": new Date(), "Title": "Объявление", "Comment": "Текст объявления" }
        ...
        this.Actions = {
            ...
            del: row => {
                ...
                if (this.BeforeDelete(data)) {
                    $.getJSON('del/Advert.dbo', { "Id": data[this.head["Id"]] }, () => {
                        row.remove();
                    });
                };
            },
...
var docTable = new CTable({ saveurl: 'edit/Advert.dbo' });
